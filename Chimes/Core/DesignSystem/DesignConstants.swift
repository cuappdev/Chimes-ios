//
//  DesignConstants.swift
//  Chimes
//
//  Created by Jay on 2/18/26.
//

import SwiftUI

struct DesignConstants {
    
    enum Fonts {
        static let H1 = Font.custom("Montserrat-SemiBold", size: 36)
        static let H2SemiBold = Font.custom("Montserrat-SemiBold", size: 18)
        static let H2Regular = Font.custom("Montserrat-Regular", size: 18)
        
        static let H3SemiBold = Font.custom("Montserrat-SemiBold", size: 16)
        static let H3Regular = Font.custom("Montserrat-Regular", size: 16)
        
        static let P = Font.custom("Inter", size: 15)
        static let Button1 = Font.custom("Inter", size: 14)
        static let Button2 = Font.custom("Inter", size: 12)
    }
}
